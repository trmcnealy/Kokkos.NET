...
namespace Kokkos::LinearAlgebra
{
    namespace Internal
    {
...
namespace Kokkos
{
    namespace FFT
    {
        __inline static void ccopy(int n, double x[], double y[])
        {
            int i;
        ...
        __inline static void step(int n, int mj, double a[], double b[], double c[], double d[], double w[], double sgn)
        {
            double ambr;
            double ambu;
            int    j;
...
#pragma omp for nowait
        ...
        __inline static void cffti(int n, double w[])
        {
            double       arg;
            double       aw;
            int          i;
...
#pragma omp for nowait
        ...
        __inline static double ggl(double* seed)
        {
...
namespace Kokkos::Extension
{
    ...
    template<class Matrix, class Vector, class Preconditioner, typename Scalar = typename Matrix::non_const_value_type>
    __inline static int ConjugateGradient(const Matrix& A, Vector& x, const Vector& b, const Preconditioner& M, int& max_iter, Scalar& tol)
    {
        ...
        Scalar normb = KokkosBlas::nrm1(b);
        ...
        if ((resid = KokkosBlas::nrm1(r) / normb) <= tol)
        {
            tol      = resid;
            max_iter = 0;
            return 0;
    ...
    template<class Matrix, class Vector, class Preconditioner, typename Scalar = typename Matrix::non_const_value_type>
    __inline static int ConjugateGradientSquared(const Matrix& A, Vector& x, const Vector& b, const Preconditioner& M, int& max_iter, Scalar& tol)
    {
        ...
        for (int i = 1; i <= max_iter; ++i)
        {
            ...
            alpha = rho_1 / KokkosBlas::dot(rtilde, vhat);
    ...
    //double NewtonRaphson(const double Target, double Start, const double Tolerance, const T& TheObject)
    //{
