...
#include <Geometry/ObjectBoundingBox.hpp>
#include <Geometry/ObjectBoundingBoxHierarchy.hpp>

#include <Kokkos_Sort.hpp>

namespace Analyzes::Trees
{

    template<typename RangeBoxType>
    class ProximitySearchTree
    {
        enum
        {
            MAX_TREE_LEVELS = 100
    ...
    public:
        ...
        ProximitySearchTree(const std::vector<RangeBoxType>& inputBoxes)
        {
            unsigned                                               numBox = inputBoxes.size();
            std::vector<Geometry::ObjectBoundingBox<RangeBoxType>> tempBoxes;
        ...
        ProximitySearchTree(std::vector<Geometry::ObjectBoundingBox<RangeBoxType>>& inputBoxes)
        {
            InitializeSearch(inputBoxes);
        }

        // Recreate new search tree from scratch
        void InitializeSearch(std::vector<Geometry::ObjectBoundingBox<RangeBoxType>>& inputBoxes)
        {
            unsigned numBox = inputBoxes.size();
            if (numBox != 0)
            {
        ...
        //
        int UpdateSearch(std::vector<Geometry::ObjectBoundingBox<RangeBoxType>>& boxes)
        {
            ...
            if (boxes.size() != numTreeBoxes)
            {
                return -1;
            }
            update_tree(m_tree.data(), boxes.data());
            return 0;
        }
        ...
        KOKKOS_FORCEINLINE_FUNCTION void SearchForOverlap(const DomainBox& searchObject, std::vector<int>& returnList) const
        {
            returnList.clear();
            if (m_tree.empty())
            {
            ...
            Geometry::ObjectBoundingBoxHierarchy<RangeBoxType> const*  current_object = m_tree.data();
            Geometry::ObjectBoundingBoxHierarchy<RangeBoxType> const*  object_stack[MAX_TREE_LEVELS];
            const Geometry::ObjectBoundingBoxHierarchy<RangeBoxType>** stack_ptr = object_stack;
        ...
        template<typename DomainBox>
        KOKKOS_FORCEINLINE_FUNCTION void SearchForOverlap(const DomainBox& searchObject, std::vector<int>& returnIndexList, std::vector<RangeBoxType>& returnBoxList)
        {
        ...
        std::vector<Geometry::ObjectBoundingBoxHierarchy<RangeBoxType>>& data()
        {
            return m_tree;
        }
        const std::vector<Geometry::ObjectBoundingBoxHierarchy<RangeBoxType>>& data() const
        {
            return m_tree;
        }

    protected:
        std::vector<Geometry::ObjectBoundingBoxHierarchy<RangeBoxType>> m_tree;
    };
...
namespace Analyzes::Trees
{
    ...
    __inline void threadedSortImpl(TView& inputVec)
    {
        Kokkos::sort<TView>(inputVec);
    ...
    __inline void threadedSort(Kokkos::View<DataType*, Layout, ExecutionSpace>& inputVec)
    {
        threadedSortImpl(inputVec);
    }

    template<typename DataType, class ExecutionSpace, class Layout = typename ExecutionSpace::array_layout>
    __inline void threadedSort(Kokkos::View<Kokkos::pair<DataType, int>, Layout, ExecutionSpace>& inputVec)
    {
        threadedSortImpl(inputVec);
    }
    ...
    KOKKOS_INLINE_FUNCTION void adjust_moment_split(const int       num_boxes,
                                                    const TDataType global_center_x,
                                                    const TDataType global_center_y,
                                                    const TDataType global_center_z,
                                                    TDataType&      moment_x,
                                                    TDataType&      moment_y,
                                                    TDataType&      moment_z)
    {
    ...
    template<typename TDataType, typename RangeBoxType>
    KOKKOS_INLINE_FUNCTION void split_boxes(const int&                                       num_boxes,
                                            const Geometry::ObjectBoundingBox<RangeBoxType>* boxes,
                                            TDataType&                                       moment_x,
                                            TDataType&                                       moment_y,
                                            TDataType&                                       moment_z)
    {
        TDataType global_center_x(0.0);
        TDataType global_center_y(0.0);
        TDataType global_center_z(0.0);
        //
...
#ifdef _OPENMP
    template<typename TDataType, typename RangeBoxType>
    KOKKOS_INLINE_FUNCTION static void split_boxes_threaded(const int                                              num_boxes,
                                                            const Geometry::ObjectBoundingBox<RangeBoxType>* const boxes,
                                                            TDataType&                                             ret_moment_x,
                                                            TDataType&                                             ret_moment_y,
                                                            TDataType&                                             ret_moment_z,
                                                            int                                                    numThreadsToUse = -1)
    {
        TDataType global_center_x(0.0);
        TDataType global_center_y(0.0);
        TDataType global_center_z(0.0);
        TDataType moment_x(0.0);
        TDataType moment_y(0.0);
        TDataType moment_z(0.0);
        //
        //  Calculate the centroid of each component box, the centroid of the current encompassing box, and the
        //  moment of inertia of all particles in the box about (0, 0, 0)
        //
...
#    endif
    ...
    //
    template<typename RangeBoxType>
    KOKKOS_INLINE_FUNCTION void store_1_node_tree(Geometry::ObjectBoundingBoxHierarchy<RangeBoxType>* root_node, const Geometry::ObjectBoundingBox<RangeBoxType>* const boxes)
    {
    ...
    template<typename RangeBoxType>
    KOKKOS_INLINE_FUNCTION void store_2_node_tree(Geometry::ObjectBoundingBoxHierarchy<RangeBoxType>* root_node, const Geometry::ObjectBoundingBox<RangeBoxType>* const boxes)
    {
        //
    ...
    template<typename TDataType, typename RangeBoxType>
    KOKKOS_INLINE_FUNCTION void store_3_node_tree(Geometry::ObjectBoundingBoxHierarchy<RangeBoxType>* root_node, const Geometry::ObjectBoundingBox<RangeBoxType>* const boxes)
    {
        //
        ...
        TDataType dist_squared_01 = std::fabs(cent0[0] - cent1[0]) + std::fabs(cent0[1] - cent1[1]) + std::fabs(cent0[2] - cent1[2]);
        TDataType dist_squared_12 = std::fabs(cent1[0] - cent2[0]) + std::fabs(cent1[1] - cent2[1]) + std::fabs(cent1[2] - cent2[2]);
        TDataType dist_squared_20 = std::fabs(cent2[0] - cent0[0]) + std::fabs(cent2[1] - cent0[1]) + std::fabs(cent2[2] - cent0[2]);
    ...
    //
    template<typename RangeBoxType>
    KOKKOS_INLINE_FUNCTION void update_tree_partial(Geometry::ObjectBoundingBoxHierarchy<RangeBoxType>*  hierarchy_data,
                                                    Geometry::ObjectBoundingBoxHierarchy<RangeBoxType>** subTreeHier,
                                                    const unsigned                                       subdivisionLevel,
                                                    unsigned                                             mySubTree)
    {
        //
        //  Terminal leaf case
        ...
        Geometry::ObjectBoundingBoxHierarchy<RangeBoxType>* left_child = hierarchy_data + 1;
        update_tree_partial(left_child, subTreeHier, subdivisionLevel / 2, mySubTree);
        Geometry::ObjectBoundingBoxHierarchy<RangeBoxType>* right_child = hierarchy_data + cur_right_child_offset;
    ...
    //
    template<typename RangeBoxType>
    KOKKOS_INLINE_FUNCTION void complete_tree_partial(Geometry::ObjectBoundingBoxHierarchy<RangeBoxType>* hierarchy_data, const unsigned subdivisionLevel)
    {
        if (subdivisionLevel == 1)
        {
    ...
    template<typename RangeBoxType>
    KOKKOS_INLINE_FUNCTION void update_tree_internal(Geometry::ObjectBoundingBoxHierarchy<RangeBoxType>* hierarchy_data, Geometry::ObjectBoundingBox<RangeBoxType>* boxes)
    {
        const int cur_right_child_offset(hierarchy_data->right_child_offset);
        if (cur_right_child_offset <= 0)
        {
    ...
    //
    template<typename RangeBoxType>
    KOKKOS_INLINE_FUNCTION void update_tree(Geometry::ObjectBoundingBoxHierarchy<RangeBoxType>* hierarchy_data, Geometry::ObjectBoundingBox<RangeBoxType>* boxes)
    {
#ifdef _OPENMP
        ...
        std::vector<Geometry::ObjectBoundingBoxHierarchy<RangeBoxType>*> sub_hierarchy_data(numLimb, nullptr);
...
#    pragma omp parallel for default(none) shared(sub_hierarchy_data, boxes, numLimb)
        ...
        {
            if (sub_hierarchy_data[i] == nullptr)
                continue;
    ...
    {
        Geometry::ObjectBoundingBoxHierarchy<RangeBoxType>* hierarchy_loc;
        Geometry::ObjectBoundingBox<RangeBoxType>*          boxes;
        Geometry::ObjectBoundingBox<RangeBoxType>*          scratch_boxes;
        int                                                 num_boxes;
        int*                                                index_array0_t;
        int*                                                index_array1_t;
        int*                                                index_array2_t;
    ...
    //
    template<typename TDataType, typename RangeBoxType>
    KOKKOS_INLINE_FUNCTION void create_hierarchy_loop(Geometry::ObjectBoundingBoxHierarchy<RangeBoxType>* hierarchy_start_ptr,
                                                      Geometry::ObjectBoundingBox<RangeBoxType>*          boxes_start_ptr,
                                                      Geometry::ObjectBoundingBox<RangeBoxType>*          scratch_boxes_start_ptr,
                                                      ...
                                                      int*                                                scratch_index2_array_start_ptr)
    {
        ...
        do
        {
            if (current_object->unwind)
            {
                ...
                //
                Geometry::ObjectBoundingBoxHierarchy<RangeBoxType>* hierarchy_loc = current_object->hierarchy_loc;
            ...
            else
            {
                ...
                else if (num_boxes <= 6)
                {
                    ...
                    Geometry::ObjectBoundingBox<RangeBoxType>* const    boxes          = current_object->boxes;
                    Geometry::ObjectBoundingBox<RangeBoxType>* const    scratch_boxes  = current_object->scratch_boxes;
                    //
...
//
#ifdef _OPENMP
    template<typename RangeBoxType>
    void create_hierarchy_loop_threaded(Geometry::ObjectBoundingBoxHierarchy<RangeBoxType>* const hierarchy_start_ptr,
                                        Geometry::ObjectBoundingBox<RangeBoxType>* const          boxes_start_ptr,
                                        Geometry::ObjectBoundingBox<RangeBoxType>* const          scratch_boxes_start_ptr,
                                        const int                                                 num_boxes_start,
                                        ...
                                        int* const                                                scratch_index2_array_start_ptr)
    {
        ...
        if (maxNumThread == 1)
        {
            create_hierarchy_loop(hierarchy_start_ptr,
                                  boxes_start_ptr,
        ...
        std::vector<Geometry::ObjectBoundingBoxHierarchy<RangeBoxType>*> limbHier(numLimb);
        std::vector<Geometry::ObjectBoundingBox<RangeBoxType>*>          limbBoxes(numLimb);
        std::vector<Geometry::ObjectBoundingBox<RangeBoxType>*>          limbScratchBoxes(numLimb);
        ...
        create_hierarchy_partial(hierarchy_start_ptr,
                                 boxes_start_ptr,
                                 scratch_boxes_start_ptr,
                                 num_boxes_start,
                                 index_array0_t_start_ptr,
...
#    pragma omp parallel for default(none)                                                                                                                                    \
        shared(numLimb, limbHier, limbBoxes, limbScratchBoxes, limbNumBoxes, limbIndex0, limbIndex1, limbIndex2, limbSIndex0, limbSIndex1, limbSIndex2)
        for (unsigned i = 0; i < numLimb; ++i)
        {
            if (limbHier[i] == nullptr)
                continue;
            create_hierarchy_loop(limbHier[i],
                                  limbBoxes[i],
                                  limbScratchBoxes[i],
                                  limbNumBoxes[i],
                                  limbIndex0[i],
    ...
    //------------------------------------------------------------------------------------------------------
    template<typename TDataType, typename RangeBoxType>
    KOKKOS_INLINE_FUNCTION void create_hierarchy(Geometry::ObjectBoundingBoxHierarchy<RangeBoxType>* hierarchy_data,
                                                 Geometry::ObjectBoundingBox<RangeBoxType>*          boxes,
                                                 const int                                           num_boxes)
    {
        //
        ...
        //  three sorted orders.
        //
        std::vector<Kokkos::pair<TDataType, int>> centroid_x(num_boxes);
        std::vector<Kokkos::pair<TDataType, int>> centroid_y(num_boxes);
        std::vector<Kokkos::pair<TDataType, int>> centroid_z(num_boxes);
#ifdef _OPENMP
#    pragma omp parallel for schedule(static) default(none) shared(centroid_x, centroid_y, centroid_z, boxes)
        ...
        for (int ibox = 0; ibox < num_boxes; ++ibox)
        {
            ...
            centroid[1]      = boxes[ibox].GetBox().get_y_min() + boxes[ibox].GetBox().get_y_max();
            centroid[2]      = boxes[ibox].GetBox().get_z_min() + boxes[ibox].GetBox().get_z_max();
            centroid_x[ibox] = Kokkos::make_pair(centroid[0], ibox);
            centroid_y[ibox] = Kokkos::make_pair(centroid[1], ibox);
            centroid_z[ibox] = Kokkos::make_pair(centroid[2], ibox);
        }
...
#    pragma omp parallel for schedule(static) default(none) shared(centroid_x, centroid_y, centroid_z, index_array0, index_array1, index_array2)
        ...
        std::vector<Geometry::ObjectBoundingBox<RangeBoxType>> scratch_boxes(num_boxes);
        //
    ...
    //
    template<typename TDataType, typename RangeBoxType>
    static void create_hierarchy_partial(Geometry::ObjectBoundingBoxHierarchy<RangeBoxType>* const hierarchy_start_ptr,
                                         Geometry::ObjectBoundingBox<RangeBoxType>* const          boxes_start_ptr,
                                         Geometry::ObjectBoundingBox<RangeBoxType>* const          scratch_boxes_start_ptr,
                                         const int                                                 num_boxes,
                                         int* const                                                index_array0_t_start_ptr,
                                         ...
                                         unsigned                                                  mySubTree,
                                         Geometry::ObjectBoundingBoxHierarchy<RangeBoxType>**      subTreeHier,
                                         Geometry::ObjectBoundingBox<RangeBoxType>**               subTreeBoxStart,
                                         Geometry::ObjectBoundingBox<RangeBoxType>**               subTreeBoxScratchStart,
                                         int*                                                      subTreeNumBoxes,
                                         int**                                                     subTreeIndex0,
                                         int**                                                     subTreeIndex1,
                                         int**                                                     subTreeIndex2,
...
#    pragma omp parallel for schedule(static) default(none) shared(master_sindex)
    ...
    //
    template<typename TDataType, typename RangeBoxType>
    KOKKOS_INLINE_FUNCTION void create_hierarchy_fork_threaded(Geometry::ObjectBoundingBoxHierarchy<RangeBoxType>* const hierarchy_start_ptr,
                                                               Geometry::ObjectBoundingBox<RangeBoxType>* const          boxes_start_ptr,
                                                               Geometry::ObjectBoundingBox<RangeBoxType>* const          scratch_boxes_start_ptr,
                                                               const int                                                 num_boxes,
                                                               int* const                                                index_array0_t_start_ptr,
