...
#define VARIANT_INLINE __inline
...
namespace System
{
    ...
    public:
        ...
        recursive_wrapper(recursive_wrapper&& operand) noexcept : p_(new T(std::move(operand.get()))) {}

        recursive_wrapper(T&& operand) : p_(new T(std::move(operand))) {}

        __inline recursive_wrapper& operator=(recursive_wrapper const& rhs)
        {
            assign(rhs.get());
            return *this;
        }

        __inline recursive_wrapper& operator=(T const& rhs)
        {
            assign(rhs);
            return *this;
        }

        __inline void swap(recursive_wrapper& operand) noexcept
        {
            T* temp    = operand.p_;
            operand.p_ = p_;
            p_         = temp;
    ...
    template<typename T>
    __inline void swap(recursive_wrapper<T>& lhs, recursive_wrapper<T>& rhs) noexcept
    {
        lhs.swap(rhs);
    }
}
...
#    endif
    ...
    namespace detail
    {
        ...
        struct unwrapper
        {
            ...
            static auto apply(typename std::remove_reference<V>::type& var) ->
                typename std::enable_if<std::is_lvalue_reference<V>::value, decltype(var.template get_unchecked<T>())>::type
            {
            ...
            template<typename V>
            static auto apply(typename std::remove_reference<V>::type& var) ->
                typename std::enable_if<!std::is_lvalue_reference<V>::value, decltype(std::move(var.template get_unchecked<T>()))>::type
            {
    ...
    public:
        ...
        VARIANT_INLINE variant(T&& val) noexcept(std::is_nothrow_constructible<typename Traits::target_type, T&&>::value) : type_index(Traits::index)
        {
    ...
    public:
        ...
        auto VARIANT_INLINE match(Fs&&... fs) const& -> decltype(variant::visit(*this, System::make_visitor(std::forward<Fs>(fs)...)))
        {
            return variant::visit(*this, System::make_visitor(std::forward<Fs>(fs)...));
        }
        // non-const
        template<typename... Fs>
        auto VARIANT_INLINE match(Fs&&... fs) & -> decltype(variant::visit(*this, System::make_visitor(std::forward<Fs>(fs)...)))
        {
            return variant::visit(*this, System::make_visitor(std::forward<Fs>(fs)...));
        }
        template<typename... Fs>
        auto VARIANT_INLINE match(Fs&&... fs) && -> decltype(variant::visit(std::move(*this), System::make_visitor(std::forward<Fs>(fs)...)))
        {
            return variant::visit(std::move(*this), System::make_visitor(std::forward<Fs>(fs)...));
        }
